...
export default function MessageComposer({
  ...
  const handleFormat = (format: string) => {
    ...
    if (format === "* " || format === "1. " || format === "> ") {
      ...
      const newText = value.substring(0, lineStart) + format + value.substring(lineStart);
      ...
      const newText = value.substring(0, start) + format + selectedText + format + value.substring(end);
  ...
  const handleEmojiSelect = (emoji: string) => {
    ...
    const newText = value.substring(0, cursorPos) + emoji + value.substring(cursorPos);
      ...
      <div className="flex-1 flex flex-col relative min-h-[300px] bg-muted/20">
        ...
        <div className="absolute bottom-2 right-2 sm:bottom-3 sm:right-3 px-2 py-1 sm:px-2.5 sm:py-1.5 bg-background/90 backdrop-blur-sm border rounded-md shadow-sm pointer-events-none">
          <div className={`flex items-center gap-1.5 sm:gap-2 text-xs ${getCounterColor()}`}>
            ...
            {showWarning && <AlertTriangle className="h-3.5 w-3.5" />}
          </div>
        </div>
      </div>
